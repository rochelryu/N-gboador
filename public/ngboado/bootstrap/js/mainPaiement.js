jQuery(document).ready(function($){
	if( $('.cd-form').length > 0 ) {
		...
		$('.cd-payment-gateways').on('change', function(){
            ...
            ($('#paypal').is(':checked'))
                ? $('#paypalCompte').velocity("slideDown", { duration: 300 })
                : $('#paypalCompte').velocity("slideUp", { duration: 300 });
            ($('#mobile_money').is(':checked'))
                ? $('#MM').velocity("slideDown", { duration: 300 })
                : $('#MM').velocity("slideUp", { duration: 300 });
		});
